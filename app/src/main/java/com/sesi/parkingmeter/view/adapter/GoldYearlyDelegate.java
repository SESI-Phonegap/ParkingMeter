...
public class GoldYearlyDelegate extends UiManagingDelegate {
    public static final String SKU_ID = "suscrip_ano";
    ...
    public void onBindViewHolder(SkuRowData data, RowViewHolder holder) {
        ...
        if (mBillingProvider.isYearlySubscribed()) {
            ...
            int textId = mBillingProvider.isSixMonthlySubscribed()
                    ? R.string.button_change : R.string.btn_suscribir;
    ...
    public void onButtonClicked(SkuRowData data) {
        if (data != null) {
            if (mBillingProvider.isSixMonthlySubscribed()) {
